...
public class ModelCW implements ActionDB {
    ...
    public ResultSet SelectData() throws Exception {
        String sql="SELECT * FROM app1";
        ResultSet rs = c.createStatement().executeQuery(sql);
        return rs;
    }

    public ResultSet SelectDataById() throws Exception{
        String sql="select * from app1 where id=?";
    ...
    public int InsertData() throws Exception {
        String sql="Insert into app1(name,surname,tel) values(?,?,?)";
        PreparedStatement stm = c.prepareStatement(sql);
    ...
    public int UpdateData() throws Exception {
        String sql="Update app1 set name=?,surname=?,tel=? where id=?";
        PreparedStatement stm = c.prepareStatement(sql);
        stm.setString(1,name);
    ...
    public int DeleteData() throws Exception {
        ...
        String sql="Delete from app1 where id=?";
        PreparedStatement stm = c.prepareStatement(sql);
        stm.setString(1,id);
        int r = stm.executeUpdate();
